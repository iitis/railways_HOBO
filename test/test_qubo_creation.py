...
def test_penalties_and_couplings():
    ...
    inds, _ = indexing4qubo(Problem.trains_paths, Problem.d_max)
    assert get_coupling(0, 0, inds, Problem) == -2.5
    assert get_coupling(0, 24, inds, Problem) == 1.25

    inds_z, l = z_indices(Problem.trains_paths, Problem.d_max)
    inds1 = list(np.concatenate([inds, inds_z]))

    k = inds1.index({'j': "j2", 's': "S1", 'd': 0})
    l = inds1.index({'j': "j1", 'j1': "j2", 's': "S2", 'd': 4, 'd1': 8})

    assert get_z_coupling(k, l, inds1, Problem) == 1.25

    assert penalty(1, inds, Problem) == 0.2
